...
ax.scatter(Usnap[:, selected_dims[0]], Usnap[:, selected_dims[1]], Usnap[:, selected_dims[2]])
...
# TO BE COMPLETED ...
U = np.dot(Usnap,eigen_vec) / np.sqrt(eigen_val)
...
# TO BE COMPLETED ...
discarded_eigenvalues = np.sum(eigen_val[Nrb+1:])
...
#
# The RB solution in the complete FE basis: Urb = B_rb. urb 
# TO BE COMPLETED ...
Urb = np.dot(B_rb, Urb)
