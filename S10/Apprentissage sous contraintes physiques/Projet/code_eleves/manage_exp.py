...
def pre_train_full(net,b_size,h_dim,x_dim,
    ...
    print(f'Empirical mean of x0: {x0.mean(dim=0)}')
...
def train_full(net, b_size, h_dim, x_dim,
    ...
    x_prev = x0  # Initialize previous x with x0
    xt = [] # a list of x, each element has a shape of (batch size,dim of x)
    yt = [] # a list of y, each element has a shape of (batch size,dim of x)

    for t in range(T + 1): 
        xt.append(x_prev)
        x_next = prop(x_prev).sample(sample_shape=torch.Size([1])).squeeze(0)
        yt.append(obs(x_next).sample(sample_shape=torch.Size([1])).squeeze(0))
        x_prev = x_next 
    ...
    def closure():
        ...
        for t in range(1, T+1):
            x_t = xt[t]  
            y_t = yt[t] 
            loss, hat = net(hat, x_t, y_t) 
            loss_total += loss
            mu_list.append(net.c(hat).mu)
        loss_total /= T
        pdf_a0 = net.c(ha0)
        logpdf_a0 = -pdf_a0.log_prob(x0).mean()
        
        loss_total = loss_total+logpdf_a0
        loss_total.backward()
        ...
        if ite == 1 or (ite % checkpoint == 0):
            ...
            # Plot the results
            plt.figure()
            plt.plot([xt[k,0,:].detach().cpu().numpy()[0] for k in range(t)], [xt[k,0,:].detach().cpu().numpy()[1] for k in range(t)], 'ro')
            plt.plot([yt[k,0,:].detach().cpu().numpy()[0] for k in range(t)], [yt[k,0,:].detach().cpu().numpy()[1] for k in range(t)], 'gx')
            plt.plot([mu_list[k][0,0].detach().cpu().numpy() for k in range(t)], [mu_list[k][0,1].detach().cpu().numpy() for k in range(t)], 'bo')
            plt.legend(["prop", "obs", "est"])
            plt.savefig(f"img/assimilation_last{ite}.png")
            plt.pause(1)
            plt.close()
