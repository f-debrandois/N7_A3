...
class DAN(nn.Module):
    ...
    def forward(self, ha, x, y):
        ...
        logpdf_a = -pdf_a.log_prob(x).mean()
        logpdf_b = -pdf_b.log_prob(x).mean()
...
class Lin2d(nn.Module):
    ...
    def __init__(self, x_dim, N, dt, init,
        ...
        # implement M
        theta = np.pi / 100
        self.M = torch.Tensor([[np.cos(theta),np.sin(theta)],
...
class Gaussian(nn.Module):
    ...
    def __init__(self, x_dim, vec, inds):
        ...
        # Fast computation of the covariance matrix
        #start = time.time() 
        ...
        #end = time.time()
        #print("Fast computation time: ", end-start)
        
        """# Slow computation of the covariance matrix
        start = time.time()
        for i in range(bs):
        ...
        print("Slow computation time: ", end-start)
        """
