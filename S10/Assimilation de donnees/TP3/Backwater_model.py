...
def J_res(model, col, domain):
    ...
    h = model(domain).clamp(min = eps)
    h_prime = torch.autograd.grad(h, domain, torch.ones_like(h), create_graph=True)[0]
    
    
    Fr_squared = q**2/(g*h**3)
    j_Ks = q**2/((Ks**2)*(h**(10/3)))
    
    res = - (b_prime + j_Ks) / (1 - Fr_squared)
    
    return torch.norm(h_prime - res, p=2)**2 / domain.shape[0]
...
def J_obs(model, obs):
    ...
    h_tilde = model(obs.data[:, 0].view(-1, 1))
    h_obs = obs.data[:, 1].view(-1, 1)
    
    return torch.norm(h_tilde - h_obs, p=2)**2 / obs.N_obs
...
def J_BC(h_tilde_BC):
    ...
    return torch.norm(h_tilde_BC - h_BC, p=2)**2
