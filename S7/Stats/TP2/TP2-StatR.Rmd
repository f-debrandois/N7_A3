...
niv.conf = c(0.95,0.90,0.80,0.50)
for (i in 1:length(n)){
  for (j in 1:length(niv.conf)){
    x= rnorm(n[i],5,2)
    IC=int.conf.moy1(x,niv.conf[j],sigma2=2)
...
```{r,eval=F}
x = rnorm(1000, 5, 2) # Echantillon simulé
...
test.moy1 <- function(x,sigma2,m0,alternative="greater"){
  ...
  if(alternative=="two.sided"){      
      pval = 2*(1 - pnorm(Zn.obs)) # A COMPLETER 
  }else{
