...
class Rosen() :
    def __init__(self) :
        print("Fonction (x,y) --> 100*(y-x²)²+(1-x)²")
        self.zeros()
    def zeros(self) :
        self.nb_eval=0 # number of evaluations of the function self.value()
        self.nb_grad=0 # number of evaluations of the function self.grad()
    ...
    def grad(self,x) :
        ...
        return np.array([2*(-1 + x[0] + 200*x[0]**3 - 200*x[0]*x[1]),200*(x[1]-x[0]**2)])
    def Hess(self,x) :
        # returns the Hessian of the function at point x
        self.nb_hess+=1
        to_return=np.zeros((2,2))
        to_return[0,0]= 2 + 1200*x[0]**2 - 400*x[1]
...
class oscill() :
    def __init__(self) :
        print("Fonction (x,y) --> 1/2*x²+xcos(y)")
        self.zeros()
    def zeros(self) :
        self.nb_eval=0 # number of evaluations of the function self.value()
        self.nb_grad=0 # number of evaluations of the function self.grad()
    ...
    def grad(self,x) :
        # returns the gradient of the function at point x
        self.nb_grad+=1
        return np.array([2*x[0]+np.cos(x[1]),-x[0]*np.sin(x[1])])
    def Hess(self,x) :
        # returns the Hessian of the function at point x
        self.nb_hess+=1
        to_return=np.zeros((2,2))
